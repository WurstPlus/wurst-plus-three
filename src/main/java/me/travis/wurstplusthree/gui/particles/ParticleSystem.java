package me.travis.wurstplusthree.gui.particles;
...
public final class ParticleSystem {
    ...
    public void render(int mouseX, int mouseY) {
        if (!Gui.INSTANCE.particles.getValue()) return;
        for (int i = 0; i < PARTS; i++) {
            final Particle particle = this.particles[i];
                for (int j = 1; j < PARTS; j++) {
                    if (i != j) {
                        ...
                        final int distance = Gui.INSTANCE.particleLength.getValue() / (scaledResolution.getScaleFactor() <= 1 ? 3 : scaledResolution.getScaleFactor());
                        if (diff < distance) {
                            final int lineAlpha = (int) map(diff, distance, 0, 0, 127);
                            if (lineAlpha > 8) {
                                RenderUtil.drawLine(particle.getPos().x + particle.getSize() / 2.0f, particle.getPos().y + particle.getSize() / 2.0f, otherParticle.getPos().x + otherParticle.getSize() / 2.0f, otherParticle.getPos().y + otherParticle.getSize() / 2.0f, 1.0f, Particle.changeAlpha(Gui.INSTANCE.particleColor.getValue().getRGB(), lineAlpha));
                            }
                        }
                    }
                }
