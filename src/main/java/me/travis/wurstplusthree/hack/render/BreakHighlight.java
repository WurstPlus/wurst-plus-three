...
public class BreakHighlight extends Hack {
    public BreakHighlight(){
        super("BreakHighlight", "Highlights where people are breaking", Category.RENDER, false);
    }
    ColourSetting self = new ColourSetting("SelfColour", new Colour(255,255,255), this);
    ColourSetting other = new ColourSetting("OtherColour", new Colour(160,0,0), this);
    ...
    public void onRender3D(Render3DEvent event){
        for(int i : breakingBlockList.keySet()){
            ...
            if(pos != null && state != -1){
                ...
                bb = calcBB(bb, state);
                if(player == mc.player){
                    RenderUtil.drawBox(bb, self.getValue(), 40 ,-1);
                }else {
                    RenderUtil.drawBox(bb, other.getValue(), 40,-1);
                }
            }
        }
    }
